...
ТАКЖЕ ИГНОРИРУЙ новости про текущий ремонт, обычную реконструкцию, реставрацию или реструктуризацию (например, ремонт терминала аэропорта, реконструкция больницы/школы, модернизация сетей), ЕСЛИ в них явно не сказано про создание новых рабочих мест. Подобные события не ведут к найму сотрудников в долгосрочной перспективе, поэтому сразу отвечай «Нет предложений». ОДНАКО, если в новости сказано, что в результате реконструкции будет существенно расширен штат и открыты новые вакансии, то новость подходит.
...
появление новых рабочих мест (например, открытие НОВОГО завода, фабрики, офиса, филиала, найм персонала);
расширение или запуск бизнеса (например, компания получила субсидию на новое производство и, вероятно, нанимает сотрудников);
...
async def handle_new_message(event):
    try:
        ...
        for news in news_blocks:
            ...
            # 2. Семантика >= 0.85 = точно дубликат (блок)
            # 3. Семантика 0.70-0.85 = серая зона -> LLM-арбитр
            # 4. Семантика < 0.70 = точно уникальная (пропускаем)
            actual_recipients = []
            for uid in target_user_ids:
                ...
                if max_sim >= DEDUP_HIGH:
                    logger.info(f"[DEDUP] User {uid} | Блок (высокое сходство {max_sim:.3f} >= {DEDUP_HIGH})")
                    continue
                elif max_sim >= DEDUP_LOW and best_text:
                    logger.info(f"[DEDUP] User {uid} | Серая зона ({max_sim:.3f}), спрашиваем LLM-арбитра...")
                    async with llm_semaphore:
                        is_dup = await llm_judge_is_duplicate(news, best_text)
                    if is_dup:
                        logger.info(f"[DEDUP] User {uid} | LLM решил: ДУБЛИКАТ")
                        continue
                    else:
                        logger.info(f"[DEDUP] User {uid} | LLM решил: УНИКАЛЬНАЯ")
