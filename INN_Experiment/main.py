...
async def llm_judge_is_duplicate(new_news: str, existing_news: str) -> bool:
    ...
    """
    judge_prompt = """Ты — суровый модератор новостной ленты. Твоя единственная цель — находить ДУБЛИКАТЫ (одно и то же событие, описанное разными словами).

ЗАДАЧА:
Сравни две новости. Если обе новости рассказывают об ОДНОМ И ТОМ ЖЕ ОСНОВНОМ СОБЫТИИ или ФАКТЕ (даже если они написаны по-разному, с разными деталями или заголовками), это ДУБЛИКАТ.

ИНСТРУКЦИЯ ПО ОТВЕТУ:
Отвечай СТРОГО одним словом:
- ДА (если это одно и то же ключевое событие/факт, просто переписанное)
- НЕТ (только если это гарантированно РАЗНЫЕ события)

Пример: "МТС открывает офис на Сахалине" и "Открытие проектного офиса МТС в регионе" -> ДА
"""
